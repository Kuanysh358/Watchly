...
namespace Watchly.Web.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> GetTmdbMovie(int tmdbId)
        {
            ...
            var videoId = await _tmdbService.GetTrailerVideoIdAsync(movie.Id)
                            ?? await _youtubeService.SearchTrailerAsync($"{movie.Title} {movie.ReleaseYear} trailer official"); return Json(new
            {
                id = movie.Id,
                title = movie.Title,
        ...
        public async Task<IActionResult> AutoImportTmdb([FromBody] AutoImportRequest? request)
        {
            ...
            foreach (var popularMovie in popularMovies)
            {
                ...
                var trailerVideoId = await _tmdbService.GetTrailerVideoIdAsync(detail.Id)
                                    ?? await _youtubeService.SearchTrailerAsync($"{detail.Title} {detail.ReleaseYear} trailer official"); var trailerUrl = string.IsNullOrWhiteSpace(trailerVideoId) ? null : _youtubeService.GetEmbedUrl(trailerVideoId);
